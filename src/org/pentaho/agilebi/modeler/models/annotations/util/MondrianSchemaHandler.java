...
public class MondrianSchemaHandler {
   ...
   */
  public void addMeasure( String cubeName, Schema.Cube.Measure measure) throws ModelerException {
    try {
      ...
      measureElement.setAttribute( MEASURE_NAME_ATTRIBUTE, measure.getName() );
      measureElement.setAttribute( MEASURE_COLUMN_ATTRIBUTE, measure.getColumn() );
      measureElement.setAttribute( MEASURE_AGGREGATOR_ATTRIBUTE,  measure.getAggregator() );
   ...
   * @throws ModelerException
   */
  public void addCalculatedMeasure( String cubeName, CalculatedMember calculatedMember ) throws ModelerException {
    try {
      XPathFactory xPathFactory = XPathFactory.newInstance();
      XPath xPath = xPathFactory.newXPath();
      StringBuffer xPathExpr = new StringBuffer();
      ...
      measureElement.setAttribute( CALCULATED_MEMBER_NAME_ATTRIBUTE, calculatedMember.getName() );
      measureElement.setAttribute( CALCULATED_MEMBER_CAPTION_ATTRIBUTE, calculatedMember.getCaption() );
      measureElement.setAttribute( CALCULATED_MEMBER_DESCRIPTION_ATTRIBUTE, calculatedMember.getDescription() );
      measureElement.setAttribute( CALCULATED_MEMBER_DIMENSION_ATTRIBUTE, calculatedMember.getDimension() );
      measureElement.setAttribute( CALCULATED_MEMBER_FORMULA_ATTRIBUTE, calculatedMember.getFormula() );
      measureElement.setAttribute( CALCULATED_MEMBER_VISIBLE_ATTRIBUTE, Boolean.toString( calculatedMember.isVisible() ) );
    } catch ( XPathExpressionException e ) {
      throw new ModelerException( e );
    }
  }
